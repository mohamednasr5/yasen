...
const ocrState = {
  capturedImage: null,        // Base64 data URL of captured image
...
function setupOcrModule() {
  ...
  if (missingElements.length > 0) {
    console.error('[OCR] Missing elements:', missingElements);
    return;
  }
  
  // Remove old listeners to prevent duplicates
  elements.captureBtn.replaceWith(elements.captureBtn.cloneNode(true));
  elements.uploadBtn.replaceWith(elements.uploadBtn.cloneNode(true));
  elements.retakeBtn.replaceWith(elements.retakeBtn.cloneNode(true));
  elements.copyBtn.replaceWith(elements.copyBtn.cloneNode(true));
  elements.pdfBtn.replaceWith(elements.pdfBtn.cloneNode(true));
  elements.printPdfBtn.replaceWith(elements.printPdfBtn.cloneNode(true));
  elements.newDocBtn.replaceWith(elements.newDocBtn.cloneNode(true));
  elements.jsonToggle.replaceWith(elements.jsonToggle.cloneNode(true));
  elements.closePdfPreview.replaceWith(elements.closePdfPreview.cloneNode(true));
  ...
  btns.capture.addEventListener('click', () => {
    console.log('[OCR] Capture button clicked');
    captureOcrImage();
  });
  
  // Upload button - trigger file input
  btns.upload.addEventListener('click', () => {
    console.log('[OCR] Upload button clicked');
  ...
  // Retake button
  btns.retake.addEventListener('click', () => {
    console.log('[OCR] Retake button clicked');
    resetOcrCapture();
  });
  
  // Copy button
  btns.copy.addEventListener('click', () => {
    console.log('[OCR] Copy button clicked');
    copyOcrText();
  });
  
  // PDF Download button
  btns.pdf.addEventListener('click', () => {
    console.log('[OCR] PDF Download button clicked');
    downloadOcrPdf();
  });
  
  // Print PDF button
  btns.printPdf.addEventListener('click', () => {
    console.log('[OCR] Print PDF button clicked');
    printOcrPdf();
  });
  
  // New document button
  btns.newDoc.addEventListener('click', () => {
    console.log('[OCR] New document button clicked');
    resetOcrFull();
  });
  ...
  btns.jsonTgl.addEventListener('click', () => {
    ...
    if (icon) {
      icon.style.transform = elements.jsonSection.classList.contains('show') ? 'rotate(180deg)' : '';
    }
  });
  
  // Close PDF Preview
  btns.closePdf.addEventListener('click', () => {
...
function downloadOcrPdf() {
  ...
  console.log('[OCR] Generating PDF...');
